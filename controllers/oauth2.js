...
server.grant(
  oauth2orize.grant.code((application, redirectUri, user, ares, callback) => {
...
server.exchange(
  oauth2orize.exchange.code((application, code, redirectUri, callback) => {
    Code.findOne({ value: code }, (err, authCode) => {
      ...
      if (application._id.toString() !== authCode.applicationId) {
        return callback(null, false);
      }
...
server.exchange(
  oauth2orize.exchange.refreshToken(
    (application, refreshToken, scope, callback) => {
