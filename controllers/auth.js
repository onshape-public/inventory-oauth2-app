...
passport.use(
  ...
  new BasicStrategy((username, password, callback) => {
    Application.findOne({ id: username }, (err, application) => {
      ...
      if (!application || application.secret !== password) {
        return callback(null, false);
      }
...
passport.use(
  new BearerStrategy((accessToken, callback) => {
    Token.findOne({ access: accessToken }, (err, token) => {
      ...
      if (
        Math.abs(token.dateModified.getTime() - now.getTime()) / 1000 >
        token.expiryTime
      ) {
        return callback(null, false);
      }
...
exports.isApplicationAuthenticated = passport.authenticate("local-part", {
  session: false,
});
